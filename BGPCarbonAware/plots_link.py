...
# Load the dataset
df = pd.read_csv("as_links_sorted.csv")
...
# --- Plot 3: Top 20 Most Polluting Links ---
df_top = df.sort_values(by="Total_CO2", ascending=False).head(20)
link_labels = df_top["AS1"].astype(str) + "–" + df_top["AS2"].astype(str)

plt.figure(figsize=(10, 6))
plt.barh(link_labels, df_top["Total_CO2"], color="darkred")
plt.title("Top 20 Most Polluting AS Links")
plt.xlabel("Total CO₂ Emissions (gCO₂)")
plt.gca().invert_yaxis()
plt.tight_layout()
plt.savefig("plot3_top20_links.png")
plt.close()
